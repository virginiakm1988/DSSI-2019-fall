...
comments %>%
    ...
    ungroup()%>%
    group_by(commentor)%>%
    mutate(avg_order = mean(cindex),tot_comment = n())%>%
    mutate(sd_order = sd(avg_order))%>%
    ungroup()%>%
...
comments %>%
    ...
    mutate(c2p = max(row_number((tag))))%>%
    summarize(n = n())%>%
    summarize(num_post = n_distinct(plink),
              
    max_c2p = max(n),
    min_c2p = min(n),
    mean_c2p = round(mean(n),digits = 2),
    
    sd_c2p = round(sd(n),digits = 2))%>%
